...
for tumorType in sorted(colorTumorTypeDict.keys()):
    ...
    dataframe = filteredDf[filteredDf["tumorHistology"] == tumorType]

    color = colorTumorTypeDict[tumorType]    

    nbL1List = dataframe["nbL1"].values
    nbSourceL1List = dataframe["nbSourceL1"].values
 
    plt.scatter(nbL1List, nbSourceL1List, c=color, edgecolor='black', linewidth='1', s=40, alpha=0.8, label=tumorType)

## Add axis labels
plt.xlabel('Number somatic L1 insertions', fontsize=12)
plt.ylabel('Number germline L1 source elements', fontsize=12)
...
corr = stats.spearmanr(filteredDf["nbL1"], filteredDf["nbSourceL1"])
...
## Save figure
outPath = outDir + '/Pictures/nbL1_nbGermlineSrc_corr.pdf'
