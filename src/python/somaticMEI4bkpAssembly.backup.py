...
for line in sourceMetadata:
    ...
    if not line.startswith("#"):
        ...
        sourceCoordOld = fieldsList[2]
        sourceMetadataDict[sourceCoordOld] = {}
        sourceMetadataDict[sourceCoordOld]["sourceCoordNew"] = sourceCoordNew 
        sourceMetadataDict[sourceCoordOld]["cytobandId"] = cytobandId
...
for line in insertions:
    ...
    if not line.startswith("#"):
        ...
        # B) Partnered transduction
            ...
            sourceCoordOld = chromSource + ":" + begSource + "-" + endSource
           
            # a) Germline source element
            if sourceCoordOld in sourceMetadataDict:
                cytobandId = sourceMetadataDict[sourceCoordOld]["cytobandId"]
                sourceType = "GERMLINE"
                sourceCoordNew = sourceMetadataDict[sourceCoordOld]["sourceCoordNew"]

                # Update source element beg and end
                chrom, coords = sourceCoordNew.split(":")
                begSource, endSource = coords.split("-")

            # b) Somatic source element
            else:
                cytobandId = "NA"
                sourceType = "SOMATIC"
