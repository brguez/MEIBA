...
dfSrcElementContributionFiltered.loc[rowName] = otherContributionSerie
print "final-contribution: ", dfSrcElementContributionFiltered
...
outFilePath = outDir + '/germline_source_element_contribution_perTumorType.tsv'
...
dfSrcElementContributionFiltered = dfSrcElementContributionFiltered.T 
dfAlleleFreqAncestryFiltered = dfAlleleFreqAncestryFiltered.T

### Convert dataframe values into floats 
dfSrcElementContributionFiltered = dfSrcElementContributionFiltered.apply(lambda x: pd.to_numeric(x))
dfAlleleFreqAncestryFiltered =  dfAlleleFreqAncestryFiltered.apply(lambda x: pd.to_numeric(x))

# Save ordered dataframes into tsv
outFilePath = outDir + '/germline_source_element_contribution_perTumorType_ordered.tsv'
dfSrcElementContributionFiltered.to_csv(outFilePath, sep='\t') 

outFilePath = outDir + '/germline_source_element_alleleFreq_perAncestry_ordered.tsv'
...
fig.suptitle('')
ax1 = sns.heatmap(dfSrcElementContributionFiltered, vmin=0, vmax=50, annot=True, fmt=".1f", linewidths=.5, cmap=plt.cm.Oranges, annot_kws={"size": 8}, square=True)
...
ax3 = projectCodeNbActiveSrcElementsSerieFiltered.plot(kind="barh")
ax3.invert_yaxis()


ax3.set(xlim=(0, 75), ylabel="",
